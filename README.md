# Samuel Rodrigues

Estudante de Ciência da Computação na UNIMAR, com foco em desenvolvimento back-end. Interesse em C/C++, sistemas embarcados, aplicações desktop e jogos.

## Tecnologias

- **Linguagens:** PHP, Python e C
- **Bibliotecas e frameworks:** pandas, Plotly, Streamlit e Tkinter
- **Ferramentas:** Git, GitHub e Composer

## Projetos

- [frete-estrategico-dados](https://github.com/samuelr-dev/frete-estrategico-dados): dashboard em Python (Streamlit) sobre cotações de frete: rede, lojas e transportadoras
- [gerador-dados-ficticios](https://github.com/samuelr-dev/gerador-dados-ficticios): aplicação desktop em Python (Tkinter) que gera dados e notícias fictícias com IA
- [estrutura-de-dados-c](https://github.com/samuelr-dev/estrutura-de-dados-c): exercícios de Estrutura de Dados em linguagem C
- [php-poo](https://github.com/samuelr-dev/php-poo): exercícios de PHP da disciplina de Programação Orientada a Objetos

## Reconhecimentos

- Vencedor do Hackathon Unimar Tech Summit 2026, com o projeto [Frete Estratégico em Dados](https://github.com/samuelr-dev/frete-estrategico-dados)

## Contato

- [LinkedIn](https://www.linkedin.com/in/samuelrodrigues-dev)
- **E-mail:** [contatosamuelrodrigues@outlook.com](mailto:contatosamuelrodrigues@outlook.com)
